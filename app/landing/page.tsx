...
export default function LandingPage() {
  ...
  useEffect(() => {
    try {
      const cp = localStorage.getItem("products_cache");
      if (cp) { setProducts(JSON.parse(cp)); setLoading(false); }
      const ci = localStorage.getItem("inventory_cache");
      if (ci) setInventory(JSON.parse(ci));
    } catch { }
      ...
      fetches.push(
        getInventory()
          .then((r) => {
            const list = Array.isArray(r.data) ? r.data : r.data.results ?? [];
            setInventory(list);
            localStorage.setItem("inventory_cache", JSON.stringify(list));
          })
          .catch(() => {}),
        getMe().then((r) => { setUsername(r.data.username || ""); setRole(r.data.role || "customer"); }).catch(() => setUsername(localStorage.getItem("username") || "")),
        getWishlist().then((r) => { const list = Array.isArray(r.data) ? r.data : r.data.results ?? []; setWishlistIds(new Set(list.map((i: { product_id: number }) => i.product_id))); }).catch(() => {}),
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentMethod === "esewa") {
        ...
        setEsewaOrderId(res.data.id); setEsewaAmount(total);
        closeOrderModal(); setEsewaModal(true);
      } else if (paymentMethod === "khalti") {
        const total = couponResult ? parseFloat(couponResult.final_amount) : getSubtotal();
        setKhaltiOrderId(res.data.id); setKhaltiAmount(total);
        closeOrderModal(); setKhaltiModal(true);
      } else {
  ...
  const checkoutProducts: CheckoutProduct[] = filtered.map((p) => {
    ...
    return {
      id: p.id, name: p.name, price: parseFloat(String(p.price)),
      category: p.category, image: p.image,
      stockStatus: normalizeStatus(rawStatus), stockQty: inv?.quantity,
    };
  });
  ...
  if (adminRedirecting) return null;
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {categories.length > 1 && (
          <div style={{ background: "var(--bg-elevated)", borderBottom: "1px solid var(--border)", padding: "8px 40px" }}>
            <AnimatedNavigationTabs
              items={categories.map((cat) => ({ id: cat, tile: cat }))}
          ...
          <section style={{ padding: "80px 40px", textAlign: "center", background: "var(--bg-elevated)", borderTop: "1px solid var(--border)" }}>
            <FadeIn>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: "12px" }}>
                <Link href="/register" style={{ padding: "13px 32px", fontSize: "15px", fontWeight: 600, color: "#fff", textDecoration: "none", borderRadius: "99px", ...glassPrimary, transition: "all 0.2s" }}>Get started</Link>
                <Link href="/login" style={{ padding: "13px 32px", fontSize: "15px", fontWeight: 500, color: "var(--text)", textDecoration: "none", borderRadius: "99px", ...glassBtn, transition: "all 0.2s" }}>Sign in</Link>
              </div>
            </FadeIn>
          </section>
        )}
      ...
      {esewaModal && esewaOrderId && <EsewaPayment orderId={esewaOrderId} amount={esewaAmount} onClose={() => setEsewaModal(false)} />}
      {khaltiModal && khaltiOrderId && <KhaltiPayment orderId={khaltiOrderId} amount={khaltiAmount} onClose={() => setKhaltiModal(false)} />}
    </div>
  );
}
